...
export default function PatientsTab({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {showCreate ? <><X size={13} strokeWidth={2} /> Cancel</> : <><UserPlus size={13} strokeWidth={2} /> New patient</>}
      </button>

      {showCreate && (
        <div className="bg-cream border border-warm-subtle rounded-2xl p-5">
          <h3 className="font-serif text-[1rem] text-warm-dark tracking-tight mb-4">Create patient</h3>
          <Form method="post" className="flex flex-col gap-4" onSubmit={() => setShowCreate(false)}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field label="Age">
                <input name="age" type="number" min={0} max={120} required className={inputCls} />
              </Field>
      ...
      {patients.length === 0 ? (
        <div className="bg-cream border border-warm-subtle rounded-2xl px-6 py-10 text-center">
          <p className="text-[0.8125rem] font-light text-warm-muted">No patients yet.</p>
        </div>
      ) : (
        patients.map((p) => (
          <div key={p._id} className="bg-cream border border-warm-subtle rounded-2xl px-5 py-4 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-4">
              <div className="flex flex-col gap-0.5">
                <span className="text-[0.9375rem] font-medium text-warm-dark">{p.name}</span>
                <span className="text-[0.75rem] text-warm-muted font-light">{p.email} · Age {p.age}</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                ...
                >
                  {editingId === p._id ? <><X size={12} strokeWidth={2} /> Close</> : <><Pencil size={12} strokeWidth={2} /> Edit</>}
                </button>
            ...
            {editingId === p._id && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Full name">
                    <input name="name" type="text" defaultValue={p.name} required className={inputCls} />
                  </Field>
                  <Field label="Email">
                    <input name="email" type="email" defaultValue={p.email} required className={inputCls} />
                  </Field>
                  <Field label="Age">
                    <input name="age" type="number" defaultValue={p.age} min={0} max={120} required className={inputCls} />
                  </Field>
