...
export default function DoctorsTab({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      >
        {showCreate ? <><X size={13} strokeWidth={2} /> Cancel</> : <><Stethoscope size={13} strokeWidth={2} /> New doctor</>}
      </button>

      {showCreate && (
        <div className="bg-cream border border-warm-subtle rounded-2xl p-5">
          <h3 className="font-serif text-[1rem] text-warm-dark tracking-tight mb-4">Create doctor</h3>
      ...
      {doctors.length === 0 ? (
        <div className="bg-cream border border-warm-subtle rounded-2xl px-6 py-10 text-center">
          <p className="text-[0.8125rem] font-light text-warm-muted">No doctors yet.</p>
        </div>
      ) : (
        doctors.map((d) => (
          <div key={d._id} className="bg-cream border border-warm-subtle rounded-2xl px-5 py-4 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-4">
              <div className="flex flex-col gap-1">
                <span className="text-[0.9375rem] font-medium text-warm-dark">{d.name}</span>
                <span className="text-[0.75rem] text-warm-muted font-light">{d.email} · {d.specialization}</span>
                <span className={`inline-flex items-center gap-1.5 text-[0.75rem] font-medium ${doctorStatusColors[d.status]}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${doctorStatusDot[d.status]}`} />
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                >
                  {editingId === d._id ? <><X size={12} strokeWidth={2} /> Close</> : <><Pencil size={12} strokeWidth={2} /> Edit</>}
                </button>
            ...
            {editingId === d._id && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Full name">
                    <input name="name" type="text" defaultValue={d.name} required className={inputCls} />
                  </Field>
                  <Field label="Email">
                    <input name="email" type="email" defaultValue={d.email} required className={inputCls} />
                  </Field>
                  <Field label="Specialization">
