...
const doctorStatusConfig = {
  available: { label: "Available", classes: "bg-green-50 text-green-600 border-green-200" },
  busy: { label: "Busy", classes: "bg-yellow-50 text-yellow-600 border-yellow-200" },
  dnd: { label: "Do not disturb", classes: "bg-red-50 text-red-600 border-red-200" },
};
...
export default function DoctorDashboard({ user, appointments }: { user: Doctor; appointments: Appointment[] }) {
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 flex items-center justify-between gap-4">
            <div className="flex flex-col gap-0.5">
              <span className="text-xs font-medium text-slate-500">Your status</span>
              <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border self-start ${doctorStatusConfig[currentStatus].classes}`}>
                {doctorStatusConfig[currentStatus].label}
              </span>
            </div>
            <div className="flex items-center gap-2">
              {(Object.keys(doctorStatusConfig) as Doctor["status"][]).map(s => (
                s !== currentStatus && (
          ...
          <div className="flex flex-col gap-3">
            ...
            {active.length === 0 ? (
              ...
              <div key={a._id} className="bg-white border border-slate-200 rounded-2xl px-5 py-4 flex items-start justify-between gap-4">
                <div className="flex flex-col gap-0.5">
          ...
          {archived.length > 0 && (
            <div className="flex flex-col gap-3">
              ...
              {showArchived && archived.map(a => (
                <div key={a._id} className="bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 flex items-start justify-between gap-4 opacity-60">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-slate-600">{a.patientId.name}</span>
                    <span className="text-xs text-slate-400">{a.department}</span>
                    <span className="text-xs text-slate-400">
                      {new Date(a.appointmentTime).toLocaleDateString()} · {new Date(a.appointmentTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </span>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full shrink-0 ${statusColors[a.status]}`}>
                    {a.status.charAt(0).toUpperCase() + a.status.slice(1)}
                  </span>
                </div>
              ))}
      ...
      {tab === "profile" && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <Form method="post" className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="specialization" className="text-xs font-medium text-slate-500">Specialization</label>
              <select id="specialization" name="specialization" defaultValue={user.specialization} required
                className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-colors">
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="password" className="text-xs font-medium text-slate-500">
                New password <span className="text-slate-300">(leave blank to keep current)</span>
              </label>
