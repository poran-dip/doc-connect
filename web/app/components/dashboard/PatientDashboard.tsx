...
import { CalendarPlus, Clock, Stethoscope, AlertTriangle, Pencil, X, ChevronDown, User } from "lucide-react";
...
function CancelModal({ onConfirm, onClose }: { onConfirm: () => void; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-warm-dark/40 backdrop-blur-sm" onClick={onClose} />
      <div className="relative bg-cream rounded-2xl p-6 shadow-2xl w-full max-w-sm mx-4 flex flex-col gap-4 border border-warm-subtle">
        <div className="flex flex-col gap-1.5">
          <h3 className="font-serif text-[1.1rem] text-warm-dark tracking-tight">Cancel appointment</h3>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-cream border border-warm-subtle rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-5">
              <CalendarPlus size={16} strokeWidth={1.75} className="text-amber" />
              <h2 className="font-serif text-[1.1rem] text-warm-dark tracking-tight">Book an appointment</h2>
            </div>
            <Form method="post" className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="appointmentTime" className={labelClass}>Date & time</label>
                  <input
                    id="appointmentTime"
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-serif text-[1.1rem] text-warm-dark tracking-tight">My appointments</h2>
            ...
            {appointments.length === 0 ? (
              <div className="bg-cream border border-warm-subtle rounded-2xl px-6 py-10 text-center">
                <p className="text-[0.8125rem] font-light text-warm-muted">No appointments yet.</p>
              </div>
            ) : (
              appointments.map((a) => (
                <div
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex flex-col gap-1">
                      <span className="text-[0.9375rem] font-medium text-warm-dark">{a.department}</span>
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                      ...
                      {a.status !== "cancelled" && a.status !== "completed" && (
                        <>
                          ...
                          <Form
                            method="post"
                            ref={cancellingId === a._id ? cancelFormRef : undefined}
                            className="hidden"
                          >
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <select name="department" defaultValue={a.department} className={inputClass}>
                            {departments.map((d) => (
                              <option key={d} value={d}>{d}</option>
                            ))}
                          </select>
      ...
      {tab === "profile" && (
        <div className="bg-cream border border-warm-subtle rounded-2xl p-6">
          ...
          <Form method="post" className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="profileAge" className={labelClass}>Age</label>
              <input id="profileAge" name="age" type="number" defaultValue={user.age} min={0} max={120} required className={inputClass} />
            </div>
            <div className="flex flex-col gap-1.5">
