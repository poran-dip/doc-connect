...
const departments = [
  ...
  "Orthopedics", "Dermatology", "Pediatrics",
];

function canEdit(a: Appointment) {
  return (a.status === "pending" || a.status === "assigned") &&
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm mx-4 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <h3 className="text-sm font-semibold text-slate-700">Cancel appointment</h3>
          <p className="text-xs text-slate-400">Are you sure you want to cancel this appointment? This cannot be undone.</p>
        </div>
        <div className="flex items-center gap-2 justify-end">
          <button type="button" onClick={onClose} className="text-xs text-slate-400 hover:text-slate-600 transition-colors cursor-pointer px-3 py-1.5">
            Keep it
          </button>
          <button type="button" onClick={onConfirm} className="text-xs bg-red-500 hover:bg-red-600 text-white font-semibold rounded-full px-4 py-1.5 transition-colors cursor-pointer">
            Yes, cancel
          </button>
        </div>
      </div>
...
export default function PatientDashboard({ user, appointments }: { user: Patient; appointments: Appointment[] }) {
        ...
        {(["appointments", "profile"] as Tab[]).map(t => (
          <button
            ...
            className={`text-xs font-medium px-4 py-1.5 rounded-full transition-colors cursor-pointer capitalize ${
              tab === t ? "bg-blue-500 text-white" : "text-slate-400 hover:text-slate-600"
            }`}
          >
            {t}
          </button>
      ...
      {tab === "appointments" && (
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6">
            <h2 className="text-sm font-semibold text-slate-700 mb-4">Book an appointment</h2>
            <Form method="post" className="flex flex-col gap-3">
              <input type="hidden" name="intent" value="book" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="flex flex-col gap-1">
                  <label htmlFor="department" className="text-xs font-medium text-slate-500">Department</label>
                  <select
                    id="department"
                    name="department"
                    required
                ...
                <div className="flex flex-col gap-1">
                  <label htmlFor="appointmentTime" className="text-xs font-medium text-slate-500">Date & time</label>
                  <input
                    id="appointmentTime"
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="emergency" name="emergency" value="true" className="accent-blue-500" />
                <label htmlFor="emergency" className="text-xs text-slate-500">Mark as emergency</label>
              </div>
              {actionData?.error && (
              ...
              <div key={a._id} className="bg-white border border-slate-200 rounded-2xl px-5 py-4 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-slate-700">{a.department}</span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {a.emergency && (
                      <span className="text-xs bg-red-50 text-red-500 font-medium px-2 py-0.5 rounded-full">Emergency</span>
                    )}
                    ...
                    {a.status !== "cancelled" && a.status !== "completed" && (
                      <>
                        ...
                        <Form method="post" ref={cancellingId === a._id ? cancelFormRef : undefined} className="hidden">
                ...
                {editingId === a._id && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor={`emergency-${a._id}`} className="text-xs text-slate-500">Mark as emergency</label>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        type="submit"
      ...
      {tab === "profile" && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          ...
          <Form method="post" className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="profileAge" className="text-xs font-medium text-slate-500">Age</label>
              <input
                id="profileAge"
                name="age"
                type="number"
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="password" className="text-xs font-medium text-slate-500">New password <span className="text-slate-300">(leave blank to keep current)</span></label>
              <input
                id="password"
                name="password"
                type="password"
