...
// ── Input helpers ──────────────────────────────────────────────────
const inputCls =
  "bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-colors w-full";
            ...
            {a.status !== "completed" &&
              ...
              availableDoctors.length > 0 && (
                ...
                >
                  ...
                  <select
                    name="doctorId"
                    className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-1.5 text-xs text-slate-700 focus:outline-none focus:border-blue-400 transition-colors flex-1"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <Field label="Status">
                    <select
                      ...
                      className={inputCls}
                    >
                      {["pending", "assigned", "completed", "cancelled"].map(
                        (s) => (
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <div className="flex flex-col gap-0.5">
                ...
                <span
                  className={`text-xs font-medium ${doctorStatusColors[d.status]}`}
                >
      ...
      <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-full p-1 self-start flex-wrap">
        {tabs.map((t) => (
          ...
          >
            ...
            {t.count !== undefined && (
              <span
                className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.key ? "bg-blue-400 text-white" : "bg-slate-100 text-slate-400"}`}
              >
                {t.count}
              </span>
            )}
